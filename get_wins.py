...
def get_driver_points_until_2017(driver_id):
    ...
    if response.status_code == 200:
        ...
        for standing in standings_list:
            ...
            if driver_standings:
                if season <= 2017:  # Accumulate points and wins until 2017
                    points_until_2017 += float(driver_standings[0].get("points", 0))
                    total_wins += int(driver_standings[0].get("wins", 0))

                # elif 2018 <= season <= 2023:  # Store points and wins for seasons 2018-2023
                #     season_points[season] = float(driver_standings[0].get("points", 0))
                #     season_wins[season] = total_wins + int(driver_standings[0].get("wins", 0))
        ...
        return {"Driver ID": driver_id, "Total Points Until 2017": 0, "Total Points": 0, "Total Wins": 0, "Season Points": {}, "Season Wins": {}}
...
def main():
    ...
    results_df.to_csv("driver_points_until_2017.csv", index=False)
